---
title: 'Animal Rising: Fighting human'
cover: https://iamge.benkstudio.com/api/cfile/AgACAgUAAyEGAASLAAFR7QADLWhRX7mang-yMJkUNwl0c7X7xnIjAAKExDEbA5-IVooO4ItzH8FQAQADAgADdwADNgQ
top_img: https://iamge.benkstudio.com/api/cfile/AgACAgUAAyEGAASLAAFR7QADLWhRX7mang-yMJkUNwl0c7X7xnIjAAKExDEbA5-IVooO4ItzH8FQAQADAgADdwADNgQ
swiper_index: 11
top_group_index: 11
background: '#fff'
date: 2025-06-17 16:45:00
updated:
tags: [2D, Tower Defense, Strategy, Web, Mobile, Unity, Blockchain, Solana, Play to Earn, Client Projects]
categories: [Game Showcase, Mobile Game]
keywords:
description: Tower defense game where animals rise up to protect their forest from ruthless pirates
top: 1
---

# 🐾 Animal Rising: Fighting human

A Play-to-Earn tower defense game where animals rise up to protect their forest from ruthless pirates. Built on the **Solana** ecosystem, Animal Defenders delivers real-time strategy gameplay combined with NFT elements and blockchain login integration.

---

## 🔥 Game Overview

The game tells the story of a battle between various animal species fighting against pirates who are attacking and destroying the forest. Players command a team of animals, deploying them strategically to defend their territory.

- 🐵 Recruit unique animal heroes
- 🪓 Fight against waves of pirate invaders
- 🌳 Upgrade tree health and production
- 💎 Earn rewards and progress through waves
---

## 🧠 Key Features

### 🔐 Solana Wallet Login
- Blockchain login via **Solana wallet**
- Data is securely managed through **PlayFab** (Microsoft backend)

### 📊 Upgrade System
Players can upgrade various attributes using in-game resources:

#### Animal Upgrades
- Unlock and level up animals with special skills
- Example: **Backattack Gator** – sneaks behind enemies and cripples them
- Stats: ❤️ 35 HP | 🗡️ 5 DMG

![Upgrade Menu](https://iamge.benkstudio.com/api/cfile/AgACAgUAAyEGAASLAAFR7QADKWhRX627mmm5l9NzlHzk5mHv8vG1AAKAxDEbA5-IVgyf8JqIognlAQADAgADdwADNgQ)

#### Stage & Resource Upgrades
- Increase food production rate and storage
- Boost health for trees and the main home tree

![Upgrade Shop](https://iamge.benkstudio.com/api/cfile/AgACAgUAAyEGAASLAAFR7QADLmhRX7t-jt6ecS89-Mg3AxVakUr1AAKFxDEbA5-IVmL2S0diwao2AQADAgADdwADNgQ)

## 🎮 Gameplay

- Tap to deploy animals
- Hold to advance forward
- Manage food production and cooldowns
- Each enemy wave becomes increasingly difficult

---

## 🔗 Demo / Download

<div style="position: relative; padding-bottom: 56.25%; height: 0; overflow: hidden; border-radius: 12px;">
  <iframe 
    src="https://www.youtube.com/embed/KLpJIaTijkw?si=dCjjznRRAIZEvVZb" 
    title="Animal Rising: Fighting human" 
    allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" 
    allowfullscreen 
    style="position: absolute; top:0; left: 0; width: 100%; height: 100%;">
  </iframe>
</div>

---

## 🖼 Screenshots

[//]: # (<img src="https://iamge.benkstudio.com/api/cfile/AgACAgUAAyEGAASLAAFR7QADLWhRX7mang-yMJkUNwl0c7X7xnIjAAKExDEbA5-IVooO4ItzH8FQAQADAgADdwADNgQ" style="width: 100%; border-radius: 8px;" />)

<div style="display: grid; grid-template-columns: repeat(2, 1fr); gap: 12px;">
  <img src="https://iamge.benkstudio.com/api/cfile/AgACAgUAAyEGAASLAAFR7QADLWhRX7mang-yMJkUNwl0c7X7xnIjAAKExDEbA5-IVooO4ItzH8FQAQADAgADdwADNgQ" style="width: 100%; border-radius: 8px;" />
  <img src="https://iamge.benkstudio.com/api/cfile/AgACAgUAAyEGAASLAAFR7QADLGhRX7aUUEai5l2N0qopz3qznWT-AAKDxDEbA5-IVjGxpeYL0sYTAQADAgADdwADNgQ" style="width: 100%; border-radius: 8px;" />
  <img src="https://iamge.benkstudio.com/api/cfile/AgACAgUAAyEGAASLAAFR7QADLmhRX7t-jt6ecS89-Mg3AxVakUr1AAKFxDEbA5-IVmL2S0diwao2AQADAgADdwADNgQ" style="width: 100%; border-radius: 8px;" />
  <img src="https://iamge.benkstudio.com/api/cfile/AgACAgUAAyEGAASLAAFR7QADK2hRX7MNWd5GO0vy1HryH0Uy9rRoAAKCxDEbA5-IVh1q6mDsRi7tAQADAgADdwADNgQ" style="width: 100%; border-radius: 8px;" />
  <img src="https://iamge.benkstudio.com/api/cfile/AgACAgUAAyEGAASLAAFR7QADKWhRX627mmm5l9NzlHzk5mHv8vG1AAKAxDEbA5-IVgyf8JqIognlAQADAgADdwADNgQ" style="width: 100%; border-radius: 8px;" />
  <img src="https://iamge.benkstudio.com/api/cfile/AgACAgUAAyEGAASLAAFR7QADKmhRX7CKpCPl1w64Zxlh8iJC7-miAAKBxDEbA5-IVlDvkBXHEo0tAQADAgADeQADNgQ" style="width: 100%; border-radius: 8px;" />
</div>

---

## 🛠 Technical Stack

| Component              | Technology / Tool                    |
|------------------------|--------------------------------------|
| Game Engine            | Unity (2D, WebGL build)              |
| Scripting Language     | C#                                   |
| Blockchain             | Solana (Wallet integration)          |
| Authentication         | Phantom Wallet (Solana)              |
| Backend                | Microsoft Playfab                    |
| Data Handling          | Cloud Save, Leaderboards via Playfab |
| Target Platforms       | Web (Desktop, Mobile)                |
| Graphics               | 2D Fantasy Style                     |
| P2E Support            | Configurable Token Rewards           |
| Version Control        | Git + GitHub                         |

---

*Post by Benk Studio Team*  
*Thientm27*
