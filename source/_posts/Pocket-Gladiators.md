---
title: 'Pocket Gladiators'
cover: https://iamge.benkstudio.com/api/cfile/AgACAgUAAyEGAASLAAFR7QADMGhRernVkCtlPJzRjGRH1V1ivTVCAALAxDEbA5-IVko_VwFgdAUlAQADAgADdwADNgQ
top_img: https://iamge.benkstudio.com/api/cfile/AgACAgUAAyEGAASLAAFR7QADMGhRernVkCtlPJzRjGRH1V1ivTVCAALAxDEbA5-IVko_VwFgdAUlAQADAgADdwADNgQ
swiper_index: 17
top_group_index: 17
background: '#fff'
date: 2025-06-17 20:00:00
updated:
tags: [2D, Fighting, Play to Earn, Blockchain, Solana, Unity, PvE, Wallet Login, Playfab, Gacha, Customization, Client Projects]
categories: [Game Showcase, Web Game]
keywords:
description: A fast-paced 2D 1v1 fighting game with RPG and gacha mechanics, built on Solana with Play to Earn integration.
top: 5
---

## ⚔️ Pocket Gladiators

**Fight. Upgrade. Earn.**

**Pocket Gladiators** is a **2D PvE fighting game** where players battle NPC opponents in intense 1vs1 matches. Each victory rewards gold, gems, and experience points — all used to gacha weapons, level up, and unlock character customizations.

This is a game that blends action with light RPG mechanics, enhanced with Web3 features for a Play to Earn experience.

---

### 🎮 Gameplay

- 🥊 **1v1 battles vs. AI**: Fast-paced duels that test reflexes and timing.
- 🏆 **Victory rewards**: Earn gold, gems, and XP after each win.
- 🎲 **Gacha system**: Use in-game currencies to roll for powerful weapons.
- 🎨 **Customization**: Change your character’s gear and appearance.
- 🔁 **Progression**: Gain XP to level up and unlock higher difficulty fights.
- 🎁 **Daily login bonus**: Come back daily for rewards and boosts.

The game’s level-based structure offers a casual but rewarding challenge curve, ideal for both quick sessions and long-term play.

---

### 🌐 Web3 Integration

- 🪙 Built on **Solana Ecosystem**
- 🧩 Login via **Solana Wallet**
- 📦 Backend powered by **Playfab (Microsoft)**
- 🪄 Token rewards & leaderboard potential
- 📊 On-chain progression tracking (optional)

The blockchain layer supports future reward systems and NFTs while keeping core gameplay accessible.

---

### 📝 Overview

With tight 2D fighting mechanics, gacha progression, and full Web3 support, **Pocket Gladiators** stands at the intersection of **arcade brawler** and **crypto-enabled reward systems**.

Key Highlights:
- 🎮 Skill-based combat with AI
- 🎰 Gacha + leveling mechanics
- 👕 Character gear & skin customization
- 🌍 WebGL support for cross-platform access
- 🪙 Play to Earn-ready with token integration
- 🏅 Leaderboards and online stat tracking via Playfab

---
### 🎥 Demo Video

<div style="position: relative; padding-bottom: 56.25%; height: 0; overflow: hidden; border-radius: 12px;">
  <iframe 
    src="https://www.youtube.com/embed/uKbou8ovv_8?si=srDdClJKy0iicu5R" 
    title="Pocket Gladiators" 
    frameborder="0" 
    allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" 
    allowfullscreen 
    style="position: absolute; top:0; left: 0; width: 100%; height: 100%;">
  </iframe>
</div>

---

### 🖼 Screenshots

<div style="display: grid; grid-template-columns: repeat(2, 1fr); gap: 12px;">
  <img src="https://iamge.benkstudio.com/api/cfile/AgACAgUAAyEGAASLAAFR7QADL2hRerQUr2U46faN7d4gxkVHEC66AAK_xDEbA5-IVumuy9LCYdRYAQADAgADdwADNgQ" style="width: 100%; border-radius: 8px;" />
  <img src="https://iamge.benkstudio.com/api/cfile/AgACAgUAAyEGAASLAAFR7QADMGhRernVkCtlPJzRjGRH1V1ivTVCAALAxDEbA5-IVko_VwFgdAUlAQADAgADdwADNgQ" style="width: 100%; border-radius: 8px;" />
  <img src="https://iamge.benkstudio.com/api/cfile/AgACAgUAAyEGAASLAAFR7QADMWhRerz6WwYu83Xaek0PCPO_85wsAALBxDEbA5-IVhJeQRFfxyXUAQADAgADdwADNgQ" style="width: 100%; border-radius: 8px;" />
  <img src="https://iamge.benkstudio.com/api/cfile/AgACAgUAAyEGAASLAAFR7QADNGhResaPV0qBIecH2ZGIY5ZywtknAALExDEbA5-IVgTVQbHwcgzRAQADAgADdwADNgQ" style="width: 100%; border-radius: 8px;" />
  <img src="https://iamge.benkstudio.com/api/cfile/AgACAgUAAyEGAASLAAFR7QADNmhResqtO4zJBWSpmyDD1Z9YkUYxAALGxDEbA5-IVrn5fbquOcX8AQADAgADdwADNgQ" style="width: 100%; border-radius: 8px;" />
  <img src="https://iamge.benkstudio.com/api/cfile/AgACAgUAAyEGAASLAAFR7QADNWhResiPGGpHEO0Mw1OrEnwCxONlAALFxDEbA5-IVvQNPVLqg1khAQADAgADdwADNgQ" style="width: 100%; border-radius: 8px;" />
  <img src="https://iamge.benkstudio.com/api/cfile/AgACAgUAAyEGAASLAAFR7QADMmhRer_ZI29HAd5ueLdqqI9dhrBmAALCxDEbA5-IVrRKEzEDZ9mnAQADAgADdwADNgQ" style="width: 100%; border-radius: 8px;" />
  <img src="https://iamge.benkstudio.com/api/cfile/AgACAgUAAyEGAASLAAFR7QADM2hResJrFiDihN84N7nzB_HJ9KGdAALDxDEbA5-IVuac6qf_PqAFAQADAgADdwADNgQ" style="width: 100%; border-radius: 8px;" />
  <img src="https://iamge.benkstudio.com/api/cfile/AgACAgUAAyEGAASLAAFR7QADN2hRes7vS4Oc5a4i3Zvcsf4UiLADAALHxDEbA5-IVvoOSG8W0g6EAQADAgADdwADNgQ" style="width: 100%; border-radius: 8px;" />
  <img src="https://iamge.benkstudio.com/api/cfile/AgACAgUAAyEGAASLAAFR7QADOWhRetMOzQpA2_R0OSzNu4QveStEAALJxDEbA5-IVsjllukYwqVtAQADAgADdwADNgQ" style="width: 100%; border-radius: 8px;" />
</div>

---

### 🛠 Technical Stack

| Component              | Technology / Tool                    |
|------------------------|--------------------------------------|
| Game Engine            | Unity (2D, WebGL build)              |
| Scripting Language     | C#                                   |
| Blockchain             | Solana (Wallet integration)          |
| Authentication         | Phantom Wallet (Solana)              |
| Backend                | Microsoft Playfab                    |
| Data Handling          | Cloud Save, Leaderboards via Playfab |
| Target Platforms       | Web (Desktop, Mobile)                |
| Graphics               | 2D Pixel/Cartoon Style               |
| P2E Support            | Configurable Token Rewards           |
| Version Control        | Git + GitHub                         |

---

*Post by Benk Studio Team*  
*Thientm27*
